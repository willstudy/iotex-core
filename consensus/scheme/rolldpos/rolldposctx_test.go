...
func TestRollDPoSCtx(t *testing.T) {
	...
	t.Run("case 1:panic because of chain is nil", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, nil, nil, nil, nil, nil, "", nil, nil)
		require.Error(err)
	})

	t.Run("case 2:panic because of rp is nil", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, nil, nil, nil, nil, "", nil, nil)
		require.Error(err)
	})
	...
	t.Run("case 3:panic because of clock is nil", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, nil, rp, nil, nil, "", nil, nil)
		require.Error(err)
	})
	...
	t.Run("case 4:panic because of fsm time bigger than block interval", func(t *testing.T) {
		_, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, nil, rp, nil, nil, "", nil, c)
		require.Error(err)
	})

	cfg.Genesis.Blockchain.BlockInterval = time.Second*20
	t.Run("case 5:normal", func(t *testing.T) {
		rctx, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), dbConfig, true, time.Second, true, b, nil, rp, nil, nil, "", nil, c)
		require.NoError(err)
		require.NotNil(rctx)
	})
}
...
func TestCheckVoteEndorser(t *testing.T) {
	...
	rctx, err := newRollDPoSCtx(consensusfsm.NewConsensusConfig(cfg), config.Default.DB, true, time.Second, true, b, nil, rp, nil, nil, "", nil, c)
	require.NoError(err)
	require.NotNil(rctx)
